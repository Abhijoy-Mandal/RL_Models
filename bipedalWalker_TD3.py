...
def get_action(means):
    ...
    salt = tf.minimum(salt, 0.1)
    salt = tf.maximum(salt, -0.1)
    actions = means+salt
...
def evaluate(eps, score):
    ...
    writer = imageio.get_writer(f'./walker_VidsTD3_v3/episode_{eps}_{score}.mp4', fps=60)
    state, _ = env.reset()
    tot_reward = 0
...
if __name__=='__main__':
    ...
    for i in range(num_episodes):
        ...
        for t in range(max_num_timesteps):
            ...
            # print(state_qn)# state needs to be the right shape for the q_network
            # loop for 4 times
            # for ech of the states we calculate moves
            # send all 4 moves at the same time
            # action = [a, b, d, c]
