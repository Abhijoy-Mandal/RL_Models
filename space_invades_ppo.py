...
def gaussian_likelihood(action, pred):
    ...
    log_std = -0.5 * tf.ones(4, dtype=np.float32)
...
def categorical_likelihood(action, pred):
    pred = tf.maximum(pred, 1e-8)
    log_pred = tf.math.log(pred)
    log_pred = action*log_pred
    return tf.cast(tf.reduce_sum(log_pred, axis=1), 'float32')
...
def compute_policy_loss(experiences, actor_network):
    ...
    LOSS_CLIPPING = 0.2
    logp = categorical_likelihood(tf.one_hot(tf.cast(actions, 'int32'), num_actions), y_pred)
...
def get_action(means):
    ...
    # salt = tf.minimum(salt, 0.25)
    # salt = tf.maximum(salt, -0.25)
    dist = tfd.Categorical(probs=means)
    actions = dist.sample()
    actions = tf.squeeze(actions)
    if random.random()<0.75:
        actions = random.randint(0, num_actions-1)
    log_pt = categorical_likelihood(tf.one_hot(actions, num_actions), means)
    return actions, log_pt

def eval_getaction(means):
    actions = tf.math.argmax(tf.squeeze(means))
    return actions
...
def evaluate(eps, score):
    max_attempts = 10000
    env = gym.make('SpaceInvaders-v4', render_mode='rgb_array', full_action_space=False)
    writer = imageio.get_writer(f'./space_invaders/episode_{eps}_{score}.mp4', fps=60)
    state, _ = env.reset()
    tot_reward = 0
...
if __name__=='__main__':
    ...
    env = gym.make('SpaceInvaders-v4', full_action_space=False, repeat_action_probability=0.25)
    ...
    curr_steps = 0
    max_score = float("-inf")
    # actor_network = tf.keras.models.load_model("walker_ppo_actor_8.h5")
    # critic_network = tf.keras.models.load_model("walker_ppo_critic_8.h5")
    while i < num_episodes:
        ...
        for j in range(batch_size):
            ...
            if done:
                ...
                if score>max_score:
                    actor_network.save('space_invaders_ppo_actor_max.h5')
                    critic_network.save('space_invaders_ppo_critic_max.h5')
                    max_score=score
                ...
                state, _ = env.reset()
                if (i+1)%1==0:
                    evaluate(i, int(av_latest_points))
                i += 1
        ...
        actor_network.save('space_invaders_ppo_actor.h5')
        critic_network.save('space_invaders_ppo_critic.h5')
    env.close()