...
policy = Sequential([
    ...
    Dense(2*num_actions, activation='tanh', kernel_regularizer=regularizers.L1(0.01))
    ### END CODE HERE ###
    ])
...
def compute_loss(experiences, gamma, q_network, target_q_network, policy_network, alt_q_net):
    ...
    # Compute max Q^(s,a)
    probs = policy_network(next_states)
    a_prime = get_action(probs)
...
def compute_policy_loss(experiences, gamma, q_network, q_network_2, policy_network):
    ...
    means = probs[:, :4]
    std_dev = probs[:, 4:]
    dist = tfd.Normal(loc=means, scale=tf.math.abs(std_dev))
    # Compute max Q^(s,a)
    action_probs = tf.math.log(dist.prob(a_prime)+0.001)
    action_probs = tf.math.reduce_prod(action_probs, axis=1)
    ### END CODE HERE ###
    # Get the q_values
    s_q = tf.concat([states, actions], axis=1)
    q_values = q_network(s_q)
    ...
    q_values_2 = tf.squeeze(q_values-2)
    q_values = tf.math.minimum(q_values_2, q_values)
    # Compute the loss
    ### START CODE HERE ###
    loss = Mean()(1e4*action_probs, q_values)
    # loss_2 = MSE(q_values_2, y_targets)
    ### END CODE HERE ###
...
def get_action(probs):
    means = probs[:, :4]
    std_devs = probs[:, 4:]
    actions = tf.random.normal([means.shape[0], num_actions], mean=means, stddev=std_devs)
...
def evaluate(eps, score):
    ...
    writer = imageio.get_writer(f'./walker_Vids3/episode_{eps}_{score}.mp4', fps=60)
    state, _ = env.reset()
