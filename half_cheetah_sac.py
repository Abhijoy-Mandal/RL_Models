...
_log_alpha = tf.Variable(-1.0)
_alpha = tfp.util.DeferredTensor(_log_alpha, tf.exp)
# _alpha = tfp.util.DeferredTensor(self._log_alpha, tf.exp)
...
experience = namedtuple("Experience", field_names=["state", "action", "reward", "next_state", "done", "log_pt"])
state_size = 17
num_actions = 6
np.random.seed(1747)
tf.random.set_seed(
...
policy = Sequential([
    ...
    Dense(2*num_actions, activation='tanh'),
    ### END CODE HERE ###
    ])
...
optimizer = Adam(learning_rate=ALPHA, clipnorm=0.5)
optimizer_2 = Adam(learning_rate=ALPHA, clipnorm=0.5)
policy_optimiser = Adam(learning_rate=POLICY_ALPHA, clipnorm=0.5)
alpha_optimiser = Adam(learning_rate=POLICY_ALPHA)

def gaussian_likelihood(action, pred, std_dev):
    # https://github.com/hill-a/stable-baselines/blob/master/stable_baselines/sac/policies.py
    # entropy_scale = tf.math.log(tf.convert_to_tensor(_alpha))
...
def compute_loss(experiences, gamma, q_network, target_q_network, policy_network, alt_q_net):
    ...
    states, actions, rewards, next_states, done_vals, _ = experiences
    entropy_scale = tf.convert_to_tensor(_alpha)
    done_vals = tf.squeeze(done_vals)
    means = policy_network(next_states)
    a_prime, log_pts = get_action(means[:, :num_actions], means[:, num_actions:])
    log_pts = tf.squeeze(log_pts)
    s_q_prime = tf.concat([next_states, a_prime], axis=1)
    target_qvals = target_q_network(s_q_prime)
    target_qvals_2 = alt_q_net(s_q_prime)
    target_qvals = tf.squeeze(tf.squeeze(tf.math.minimum(target_qvals, target_qvals_2)) - entropy_scale*log_pts)
    ### START CODE HERE ###
...
def compute_policy_loss(experiences, q_network_2, q_network, policy_network):
    ...
    mu_s, log_pts = get_action(means[:, :num_actions], means[:, num_actions:])
    mu_s = tf.squeeze(mu_s)
    log_pts = tf.squeeze(log_pts)
    ...
    loss = -1.0*tf.nn.compute_average_loss(tf.squeeze(tf.math.minimum(q_values, q_values2)) - entropy_scale*log_pts)
    ### END CODE HERE ###
    return loss


# def alpha_loss(experiences):
...
def alpha_learn(experiences):
    states, actions, rewards, next_states, done_vals, log_pts = experiences
    with tf.GradientTape() as tape:
        alpha_losses = -1.0 * (
                _alpha * tf.stop_gradient(log_pts + 6))
    ...
    alpha_gradients = tape.gradient(alpha_loss, [_log_alpha])
    alpha_optimiser.apply_gradients(zip(alpha_gradients, [_log_alpha]))
...
def get_action(means, log_std_dev):
    # salt = tf.random.normal([means.shape[0], num_actions], mean=0, stddev=tf.convert_to_tensor(_alpha))
    # salt = tf.minimum(salt, abs_salt)
    # salt = tf.maximum(salt, -abs_salt)
    std_dev = tf.cast(tf.exp(log_std_dev), 'float32')
    actions = tf.random.normal([means.shape[0], num_actions], mean=means, stddev=std_dev)
    actions = tf.squeeze(actions)
    actions = tf.minimum(actions, 1)
    actions = tf.maximum(actions, -1)
    means = tf.squeeze(means)
    log_pt = gaussian_likelihood(actions, means, log_std_dev)
    return actions, log_pt

def check_update_conditions(t, NUM_STEPS_FOR_UPDATE, memory_buffer):
    if t%NUM_STEPS_FOR_UPDATE==0 and len(memory_buffer)>1000:
        return True
    return False
...
def get_experiences(memory_buffer):
    sample = random.sample(memory_buffer, 256)
    states = []
    actions = []
    rewards = []
    next_states = []
    ...
    log_pts = []
    for exp in sample:
        state, action, reward, next_state, done_val,log_pt = exp
        states.append(state)
    ...
    return np.array(states).astype('float32'), np.array(actions).astype('float32'), np.array(rewards).astype('float32'), np.array(next_states).astype('float32'), np.array(done_vals).astype('float32'), np.array(log_pts).astype('float32')


def evaluate(eps, score, max_score):
    max_attempts = 1000
    env = gym.make("HalfCheetah-v4", render_mode="rgb_array")
    writer = imageio.get_writer(f'./half_cheetah_sac/episode_{eps}_{score}_{max_score}.mp4', fps=30)
    state, _ = env.reset()
    tot_reward = 0
    for i in range(max_attempts):
        ...
        action = eval_getaction(dist[:, :num_actions])
        action = tf.squeeze(action)
        env_action = 1.0*action
...
if __name__=='__main__':
    ...
    # policy = tf.keras.models.load_model('humanoid_policy_td3_3.h5')
    # q_network = tf.keras.models.load_model('humanoid_q1_td3_3.h5')
    # q_network_2 = tf.keras.models.load_model('humanoid_q2_td3_3.h5')
    ...
    for i in range(num_episodes):
        ...
        for t in range(max_num_timesteps):
            ...
            dist = policy(state_qn)
            action, log_pt = get_action(dist[:, :num_actions], dist[:, num_actions:])
            entropy+=log_pt[0]
            action = tf.squeeze(action)
            env_action = 1.0*action
            # Take action A and receive reward R and the next state S'
            ...
            memory_buffer.append(experience(state, action, reward, next_state, done, log_pt[0]))
            ...
            if done:
                break
        entropy = entropy/(episode_time_step)
        if total_points>max_score:
            max_score=total_points
            policy.save('HalfCheetah_policy_sac_max1.h5')
            q_network.save('HalfCheetah_q1_sac_max1.h5')
            q_network_2.save('HalfCheetah_q2_sac_max1.h5')
        total_point_history.append(total_points)
        av_latest_points = np.mean(total_point_history[-num_p_av:])
        tot_ep_time = time.time()-episode_time_start
        print(f"\rEpisode {i + 1} | Total point average of the last {num_p_av} episodes: {av_latest_points:.2f} episode_time:{tot_ep_time:.2f}, latest reward:{total_points}, max score:{max_score}, entropy:{entropy}",
              end=" ")
        tf.print(_alpha)
        if (i + 1) % 10 == 0:
            print(f"Episode {i + 1} | Total point average of the last {num_p_av} episodes: {av_latest_points:.2f}")
            policy.save('HalfCheetah_policy_sac_1.h5')
            q_network.save('HalfCheetah_q1_sac_1.h5')
            q_network_2.save('HalfCheetah_q2_sac_1.h5')
        # We will consider that the environment is solved if we get an
        # average of 200 points in the last 100 episodes.
        if (i+1)%eval_freq == 0:
            print("Evaluating...")
            for k in range(1):
                evaluate(i+k+ep_offset, int(av_latest_points), int(max_score))
