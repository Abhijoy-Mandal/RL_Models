...
def compute_loss(experiences, gamma, q_network, target_q_network, policy_network, alt_q_net):
    ...
    a_prime, _ = get_action(means, 0.4)
...
def get_action(means, abs_salt):
    ...
    actions = tf.minimum(actions, 2)
    actions = tf.maximum(actions, -2)
...
def eval_getaction(means):
    ...
    actions = tf.minimum(actions, 2.0)
    actions = tf.maximum(actions, -2.0)
    return actions
...
def evaluate(eps, score):
    ...
    writer = imageio.get_writer(f'./pusher_vids/episode_{eps}_{score}.mp4', fps=30)
    state, _ = env.reset()
    tot_reward = 0
...
if __name__=='__main__':
    ...
    # policy = tf.keras.models.load_model('pusher_policy_td3.h5')
    # q_network = tf.keras.models.load_model('pusher_q1_td3.h5')
    # q_network_2 = tf.keras.models.load_model('pusher_q2_td3.h5')
    ...
    for i in range(num_episodes):
        ...
        if (i + 1) % num_p_av == 0:
            ...
            policy.save('pusher_policy_td3.h5')
            q_network.save('pusher_q1_td3.h5')
            q_network_2.save('pusher_q2_td3.h5')
        # We will consider that the environment is solved if we get an
        # average of 200 points in the last 100 episodes.
        if (i+1)%eval_freq == 0:
            print("Evaluating...")
            evaluate(i+1+ep_offset, int(av_latest_points))
