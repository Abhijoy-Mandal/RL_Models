...
def gaussian_likelihood(action, pred):
    ...
    log_std = -0.5 * tf.ones(17, dtype=np.float32)
...
def evaluate(eps, score):
    ...
    writer = imageio.get_writer(f'./humanoid-vids/episode_{eps}_{score}.mp4', fps=60)
    state, _ = env.reset()
    tot_reward = 0
    for i in range(max_attempts):
        ...
        img = np.asarray(Image.fromarray(np.asarray(env.render())).resize((500, 500)))
...
if __name__=='__main__':
    ...
    curr_steps = 0
    max_score = float("-inf")
    actor_network = tf.keras.models.load_model("humanoid_ppo_actor_max_3.h5")
    critic_network = tf.keras.models.load_model("humanoid_ppo_critic_max_3.h5")
    while i < num_episodes:
        ...
        for j in range(batch_size):
            ...
            if done:
                ...
                if score>max_score:
                    actor_network.save('humanoid_ppo_actor_max_4.h5')
                    critic_network.save('humanoid_ppo_critic_max_4.h5')
                    max_score=score
                state, _ = env.reset()
        ...
        actor_network.save('humanoid_ppo_actor_2.h5')
        critic_network.save('humanoid_ppo_critic_2.h5')
    env.close()