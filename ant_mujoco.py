...
def get_action(means):
    ...
    salt = tf.minimum(salt, 0.4)
    salt = tf.maximum(salt, -0.4)
    actions = means+salt
...
def evaluate(eps, score):
    max_attempts = 1000
    env = gym.make("Ant-v4", render_mode="rgb_array", healthy_reward=0.1, ctrl_cost_weight=0, healthy_z_range=(0.3, 1))
    writer = imageio.get_writer(f'./and_mujoco/episode_{eps}_{score}.mp4', fps=60)
    state, _ = env.reset()
    tot_reward = 0
...
if __name__=='__main__':
    ...
    env = gym.make("Ant-v4", healthy_reward=0.1, ctrl_cost_weight=0, healthy_z_range=(0.3, 1))
    ...
    curr_steps = 0
    max_score = float("-inf")
    # actor_network = tf.keras.models.load_model("ant_ppo_actor_5.h5")
    # critic_network = tf.keras.models.load_model("ant_ppo_critic_5.h5")
    while i < num_episodes:
        ...
        for j in range(batch_size):
            ...
            if done:
                ...
                if score>max_score:
                    actor_network.save('ant_ppo_actor_max5.h5')
                    critic_network.save('ant_ppo_critic_max5.h5')
                    max_score=score
                state, _ = env.reset()
        ...
        actor_network.save('ant_ppo_actor_6.h5')
        critic_network.save('ant_ppo_critic_6.h5')
    env.close()