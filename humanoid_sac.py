...
optimizer_2 = Adam(learning_rate=ALPHA, clipnorm=0.25)
...
def compute_loss(experiences, gamma, q_network, target_q_network, policy_network, alt_q_net):
    ...
    a_prime, log_pts = get_action(means, 0)
...
def compute_policy_loss(experiences, q_network_2, q_network, policy_network):
    ...
    mu_s, log_pts = get_action(means, 0)
...
def alpha_learn(experiences):
    ...
    with tf.GradientTape() as tape:
        alpha_losses = -1.0 * (
                _alpha * tf.stop_gradient(log_pts + 15.15))
...
def get_action(means, log_std_dev):
    ...
    actions = tf.random.normal([means.shape[0], num_actions], mean=means, stddev=0.6)
    actions = tf.squeeze(actions)
    actions = tf.minimum(actions, 1)
    actions = tf.maximum(actions, -1)
    log_pt = gaussian_likelihood(actions, means, 0)
    return actions, log_pt

def check_update_conditions(t, NUM_STEPS_FOR_UPDATE, memory_buffer):
    if t%NUM_STEPS_FOR_UPDATE==0 and len(memory_buffer)>20000:
        return True
    return False
...
if __name__=='__main__':
    ...
    policy = tf.keras.models.load_model('humanoid_policy_sac_6.h5')
    q_network = tf.keras.models.load_model('humanoid_q1_sac_6.h5')
    q_network_2 = tf.keras.models.load_model('humanoid_q2_sac_6.h5')
    ...
    for i in range(num_episodes):
        ...
        for t in range(max_num_timesteps):
            ...
            dist = policy(state_qn)
            action, log_pt = get_action(dist, 0)
            entropy+=log_pt[0]
            action = tf.squeeze(action)
            env_action = 0.4*action
            # Take action A and receive reward R and the next state S'
            ...
            if done:
                break
        entropy = entropy/(time_step-ini_time)
        if total_points>max_score:
            max_score=total_points
            policy.save('humanoid_policy_sac_max7.h5')
            q_network.save('humanoid_q1_sac_max7.h5')
            q_network_2.save('humanoid_q2_sac_max7.h5')
        ...
        if (i + 1) % 10 == 0:
            ...
            policy.save('humanoid_policy_sac_7.h5')
            q_network.save('humanoid_q1_sac_7.h5')
            q_network_2.save('humanoid_q2_sac_7.h5')
        # We will consider that the environment is solved if we get an
        # average of 200 points in the last 100 episodes.
        if (i+1)%eval_freq == 0:
            print("Evaluating...")
            for k in range(1):
                evaluate(i+1+k+ep_offset, int(av_latest_points), int(max_score))
