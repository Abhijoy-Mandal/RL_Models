...
NUM_STEPS_FOR_UPDATE = 32  # perform a learning update every C time steps
...
def evaluate(eps, score):
    ...
    writer = imageio.get_writer(f'./mountainCarVids6/episode_{eps}_{score}.mp4', fps=60)
    state, _ = env.reset()
...
if __name__=='__main__':
    ...
    for i in range(num_episodes):
        ...
        for t in range(max_num_timesteps):
            ...
            # highest = max(next_state[0], highest)
            # reward = highest
            if next_state[0]<-0.5:
                reward+=0
        ...
        if (i + 1) % num_p_av == 0:
            ...
            action_list = [0] * num_actions
            q_network.save('mountainCar_v16_simple.h5')
